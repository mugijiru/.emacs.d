...
(defun el-get-lock-update-check-execute (&optional only-obsolute-count)
  (let ((lists (el-get-lock-update-check-assemble-lists)))
    (if only-obsolute-count
      ...
      (el-get-lock-update-check-print-list2 "not installed" (alist-get 'not-installed lists))
      (el-get-lock-update-check-print-list2 "cannot get hash" (alist-get 'cannot-get-hash lists))
      (el-get-lock-update-check-print-list2 "emacswiki" (alist-get 'emacswiki lists))
      (el-get-lock-update-check-print-list2 "cannot-get-url" (alist-get 'cannot-get-url lists)))
    ))
