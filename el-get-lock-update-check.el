(progn
  ...
  (let ((obsolute-packages '())
                      ...
                      (progn
                        (let* ((grep-regexp-prefix "refs/")
                          ...
                          (if hash
                            ...
                            (add-to-list 'not-checkable-packages package))
                          ))
                    (add-to-list 'not-checkable-packages package))))
    ...
    (dolist (package not-installed-packages)
      (message package))
    (message "\n## not checkable packages ##")
    (dolist (package not-checkable-packages)
      (message package))))
