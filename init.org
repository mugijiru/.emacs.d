* これは何?
  ...
  なお、記載している順番と実際に読み込まれる順番は違うという罠がある。
  init-loader で読み込み順を制御しているので
  ファイルの prefix になってる数字の順に読み込まれるようになっている。
...
** Molokai Theme
   ...
   今使ってるテーマは Molokai なのです。
   https://github.com/alloy-d/color-theme-molokai

   元々は monokai を使っていたけど、何かの理由で乗り換えた。
   テーマの形式が古かったとかそんな理由だった気がする。

   ファイルは 90-theme.el と init-loader 的にも大分後ろの方で設定している。
   早い内にテーマを読み込むと行儀の悪い何かに上書きされるとか、そんな理由だったはず。

*** インストール

    el-get-lock で lock できるようにしたかったから、
    MELPA からではなく GitHub から入れるようにしている。

    #+begin_src emacs-lisp :tangle inits/90-theme.el
    (el-get-bundle alloy-d/color-theme-molokai)
    #+end_src

    まあ lock が目的というより、今使ってる revision がわかるようにしたいってだけなんだけど
